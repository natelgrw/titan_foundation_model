...
def dict_to_graph_data(data_dict, edge_type_vocab):
    ...
    # spec vector
    spec_keys = ["gain", "UGBW", "PM", "power"]
    spec_vector = [specs.get(k, 0.0) for k in spec_keys]
    spec_tensor = torch.tensor(spec_vector, dtype=torch.float)
    ...
    return {
        ...
        "sizing_targets": sizing_target_tensor,
        "spec": spec_tensor
    }
...
dataset = [dict_to_graph_data(example_dict, EDGE_TYPE_VOCAB) for example_dict in list_of_circuits]
